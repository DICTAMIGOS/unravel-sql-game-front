...
interface StoryImageProps {
  ...
  onNext: () => void;
  className?: string;
  /** Diálogos (string[] o { text, durationMs }[]) que vienen desde challenges */
  dialogs?: DialogItem[];
  /** Oculta el botón y avanza solo al terminar los diálogos */
  ...
  return (
   ...
   intervalMs={10000} // 10 segundos
          ...
          {image.caption && (
            ...
            >
              <p className="text-gray-300 text-lg leading-relaxed">{image.caption}</p>
            </motion.div>
          )}

          {/* Botón Continuar (útil si no hay diálogos o quieres avance manual) */}
          {!hideContinueButton && (
            <motion.div
              initial={{ opacity: 0, y: 10 }}
              animate={{ opacity: 1, y: 0 }}
